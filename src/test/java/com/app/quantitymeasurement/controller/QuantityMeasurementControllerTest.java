...
class QuantityMeasurementControllerTest {
	...
	void testAddQuantities() throws Exception {
		...
		Mockito.when(service.addQuantities(any(QuantityDTO.class), any(QuantityDTO.class), any())).thenReturn(result);
...
//				.andExpect(jsonPath("$.resultUnit").value("FEET"));
	...
	void testConvertQuantity() throws Exception {
		...
		Mockito.when(service.convertQuantity(any(QuantityDTO.class), any(QuantityDTO.class), any())).thenReturn(result);
	...
	void testSubtractQuantities() throws Exception {
		...
		mockMvc.perform(post("/api/v1/quantities/subtract").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(input))).andExpect(status().isOk())
				.andExpect(jsonPath("$.operation").value("subtract")).andExpect(jsonPath("$.resultValue").value(3.0));
	}
	...
	void testDivideQuantities() throws Exception {
		...
		mockMvc.perform(post("/api/v1/quantities/divide").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(input))).andExpect(status().isOk())
				.andExpect(jsonPath("$.operation").value("divide")).andExpect(jsonPath("$.resultValue").value(3.0));
	}
