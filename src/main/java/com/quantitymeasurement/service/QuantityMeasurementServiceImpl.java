...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
    ...
    public QuantityDTO compare(QuantityDTO quantity1, QuantityDTO quantity2) {
        try {
            ...
            QuantityMeasurementEntity entity =
                    new QuantityMeasurementEntity(model1, model2, "COMPARE", result);

            repository.save(entity);

            logger.info("Comparison executed successfully");
    ...
    public QuantityDTO convert(QuantityDTO quantity, String targetUnit) {
        ...
        try {
            ...
            IMeasurable sourceUnit =
                    IMeasurable.getUnitByName(quantity.getUnit(), quantity.getMeasurementType());

            IMeasurable targetUnitObj =
                    IMeasurable.getUnitByName(targetUnit, quantity.getMeasurementType());

            QuantityModel<IMeasurable> sourceModel =
                    new QuantityModel<>(quantity.getValue(), sourceUnit);

            double convertedValue =
                    Quantity.convert(quantity.getValue(), sourceUnit, targetUnitObj);

            QuantityModel<IMeasurable> resultModel =
                    new QuantityModel<>(convertedValue, targetUnitObj);

            QuantityMeasurementEntity entity =
                    new QuantityMeasurementEntity(sourceModel, null, "CONVERT", resultModel);
    ...
    public QuantityDTO add(QuantityDTO quantity1, QuantityDTO quantity2, String targetUnit) {
        ...
        try {
            ...
            IMeasurable unit1 =
                    IMeasurable.getUnitByName(quantity1.getUnit(), quantity1.getMeasurementType());

            IMeasurable unit2 =
                    IMeasurable.getUnitByName(quantity2.getUnit(), quantity2.getMeasurementType());

            IMeasurable targetUnitObj =
                    IMeasurable.getUnitByName(targetUnit, quantity1.getMeasurementType());

            QuantityModel<IMeasurable> model1 =
                    new QuantityModel<>(quantity1.getValue(), unit1);

            QuantityModel<IMeasurable> model2 =
                    new QuantityModel<>(quantity2.getValue(), unit2);
            ...
            QuantityMeasurementEntity entity =
                    new QuantityMeasurementEntity(model1, model2, "ADD", resultModel);

            repository.save(entity);

            logger.info("Addition executed successfully");
    ...
    public QuantityDTO subtract(QuantityDTO quantity1, QuantityDTO quantity2, String targetUnit) {
        ...
        try {
            ...
            QuantityMeasurementEntity entity =
                    new QuantityMeasurementEntity(model1, model2, "SUBTRACT", resultModel);

            repository.save(entity);

            logger.info("Subtraction executed successfully");
    ...
    public QuantityDTO divide(QuantityDTO quantity1, QuantityDTO quantity2) {
        ...
        try {
            ...
            QuantityMeasurementEntity entity =
                    new QuantityMeasurementEntity(model1, model2, "DIVIDE", String.valueOf(result));

            repository.save(entity);

            logger.info("Division executed successfully");
    ...
    private void validateSameCategory(QuantityDTO quantity1, QuantityDTO quantity2) {
        ...
        if (!quantity1.getMeasurementType().equals(quantity2.getMeasurementType())) {
            ...
            throw new IllegalArgumentException(
                    "Cannot perform operation across different measurement categories: "
                            + quantity1.getMeasurementType()
                            + " and "
                            + quantity2.getMeasurementType());
        }
    }
